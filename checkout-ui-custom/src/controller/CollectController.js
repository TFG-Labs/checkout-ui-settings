...
const CollectController = (() => {
  ...
  const updateCollectSummary = (recipient) => {
    if (!recipient) return;
    const [name, number] = recipient.split(RECIPIENT_DELIMITER);
    ...
    $('.collect-receiver').html(`${name} <br /> ${number} `);
  };

  const saveCollectFields = () => {
    checkForm();

    if (state.validForm && $('#pickup-receiver').val() && $('#custom-pickup-complement').val()) {
      const receiverName = `${$('#pickup-receiver').val()}`;
      const phone = $('#custom-pickup-complement').val().replace(/\s/g, '');

      localStorage.setItem('saving-shipping-collect', true);
      $('#btn-go-to-payment').trigger('click');
      const { shippingData } = window.vtexjs.checkout.orderForm;
      updateCollectSummary(receiverName);

      setTimeout(async () => {
        shippingData.address.receiverName = receiverName;
        shippingData.selectedAddresses[0].receiverName = receiverName;

        console.info('SAVE COLLECT', { receiverName, shippingData });

        sendOrderFormCustomData(PICKUP_APP, { phone }).then((data) => {
          console.info('PICKUP SENT', { data });
        });

        // window.vtexjs.checkout.calculateShipping(shippingData.address).done(() => {
  ...
  const setInputPhone = () => {
    const phoneInput = document.querySelector('#custom-pickup-complement');
  ...
  const prePopulateReceiverFields = () => {
    const receiverName = getBestRecipient('pickup-receiver');
    const receiverPhone = getBestPhoneNumber('custom-pickup-complement');

    let populatedReceiverName = receiverName + RECIPIENT_DELIMITER + receiverPhone;

    const savedReceiverName = window.vtexjs.checkout?.orderForm?.shippingData?.address?.receiverName || '';

    if (savedReceiverName.includes(RECIPIENT_DELIMITER)) populatedReceiverName = savedReceiverName;

    if ($('#pickup-receiver').length && $('#pickup-receiver').val().trim() === '') {
      $('#pickup-receiver').val(receiverName);
    }
    if ($('#custom-pickup-complement').length && $('#custom-pickup-complement').val().trim() === '') {
      $('#custom-pickup-complement').val(receiverPhone);
    }

    if ($('#pickup-receiver').val() === '' || savedReceiverName !== populatedReceiverName) {
      // POpulate vtex receiver field
      // $('#pickup-receiver').val(populatedReceiverName);

      // Update the order form if it's incorrect
      window.vtexjs.checkout.getOrderForm().then((orderForm) => {
        const { shippingData } = orderForm;
        if (!shippingData || !shippingData.address) return {};

        shippingData.address.receiverName = populatedReceiverName;
        shippingData.selectedAddresses[0].receiverName = populatedReceiverName;
  ...
  const addCustomPhoneInput = () => {
    ...
    const phoneNumber = getBestPhoneNumber('custom-pickup-complement');

    prePopulateReceiverFields();
    ...
    if ($('#custom-pickup-complement').length === 0) {
      ...
      if (phoneNumber) {
        $('#custom-pickup-complement').val(phoneNumber);
      }
    } else if ($('#custom-pickup-complement').val() === '') {
      $('#custom-pickup-complement').val(phoneNumber);
    }
  };
  ...
  const fixBadShippingData = () => {
    const hasBadShippingData = $('.shipping-summary-info').text() === 'Waiting for more information';
    if (hasBadShippingData) {
    ...
    if (window.location.hash === STEPS.SHIPPING && shippingLoaded) {
      ...
      if (state.inCollect) {
        ...
        if (state.pickupSelected && !state.collectReset) {
          ...
          $('#box-pickup-complement').addClass('show');

          addCustomPhoneInput();
          addCustomBtnPayment();
        } else {
          $('.shp-pickup-receiver').removeClass('show');
          $('#box-pickup-complement').removeClass('show');
        }
